...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/owner/dashboard/stats", async (req: any, res) => {
    try {
      const user = req.user;
      if (!user) {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const stats = await storage.getOwnerDashboardStats(organizationId, ownerId, period, customRange);
      res.json(stats);
    } catch (error) {
      console.error("Error fetching owner dashboard stats:", error);
      res.status(500).json({ message: "Failed to fetch dashboard stats" });
  ...
  app.get("/api/owner/activity", async (req: any, res) => {
    try {
      const user = req.user;
      if (!user) {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const activity = await storage.getOwnerActivity(
        organizationId,
        ownerId,
        propertyId ? parseInt(propertyId) : undefined,
        type
  ...
  app.get("/api/owner/ai-suggestions", async (req: any, res) => {
    try {
      const user = req.user;
      if (!user) {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const suggestions = await storage.getOwnerAISuggestions(organizationId, ownerId);
  ...
  app.get("/api/owner/financial-breakdown", async (req: any, res) => {
    try {
      const user = req.user;
      if (!user) {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const breakdown = await storage.getOwnerFinancialBreakdown(organizationId, ownerId, period, customRange);
      res.json(breakdown);
    } catch (error) {
  ...
  app.patch("/api/owner/ai-suggestions/:id", async (req: any, res) => {
    try {
      const user = req.user;
      if (!user) {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const updated = await storage.updateAISuggestionStatus(
        organizationId,
        suggestionId,
        status,
        user.id
  ...
  // Log Owner Activity
  app.post("/api/owner/activity", async (req: any, res) => {
    try {
      const user = req.user;
      if (!user) {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const activity = await storage.logOwnerActivity(organizationId, ownerId, activityData);
      res.json(activity);
    } catch (error) {
      console.error("Error logging owner activity:", error);
