...
export class DatabaseStorage implements IStorage {
  ...
  async getOwnerDashboardStats(organizationId: string, ownerId: string, period: string, customRange?: { start: string; end: string }): Promise<any> {
    ...
    // Get current balance from owner payouts
    ...
    const avgRateResult = await db.select({
      avgRate: sql<number>`COALESCE(AVG(total_amount / NULLIF(EXTRACT(days FROM (check_out - check_in)), 0)), 0)`
    })
    .from(bookings)
    .where(and(
    ...
    const earningsResult = await db.select({
      thisMonth: sql<number>`COALESCE(SUM(CASE WHEN created_at >= ${thisMonthStart.toISOString()} THEN amount ELSE 0 END), 0)`,
      thisYear: sql<number>`COALESCE(SUM(CASE WHEN created_at >= ${thisYearStart.toISOString()} THEN amount ELSE 0 END), 0)`,
      customPeriod: sql<number>`COALESCE(SUM(CASE WHEN created_at BETWEEN ${startDate.toISOString()} AND ${endDate.toISOString()} THEN amount ELSE 0 END), 0)`
    })
    ...
    .where(and(
      eq(bookings.organizationId, organizationId),
      inArray(bookings.propertyId, propertyIds),
      gte(bookings.checkIn, new Date().toISOString()),
      eq(bookings.status, 'confirmed')
    ));

    // Get booking sources breakdown
    const bookingSources = await db.select({
    ...
    bookingSources.forEach(source => {
      if (source.source && source.source in sourcesMap) {
        sourcesMap[source.source as keyof typeof sourcesMap] = source.count;
      }
    });

    return {
      currentBalance: balanceResult[0]?.totalEarnings || 0,
      averageNightlyRate: avgRateResult[0]?.avgRate || 0,
      totalEarnings: {
        thisMonth: earningsResult[0]?.thisMonth || 0,
        thisYear: earningsResult[0]?.thisYear || 0,
        customPeriod: earningsResult[0]?.customPeriod || 0
      },
      upcomingBookings: upcomingBookings[0]?.count || 0,
      bookingSources: sourcesMap,
      properties: userProperties
    };
  }

  // Get owner activity timeline
  async getOwnerActivity(organizationId: string, ownerId: string, propertyId?: number, activityFilter?: string): Promise<any[]> {
    let query = db.select({
      id: ownerActivity.id,
      type: ownerActivity.type,
      title: ownerActivity.title,
      description: ownerActivity.description,
      timestamp: ownerActivity.createdAt,
      propertyId: ownerActivity.propertyId,
      propertyName: properties.name,
      metadata: ownerActivity.metadata
    })
    .from(ownerActivity)
    .leftJoin(properties, eq(ownerActivity.propertyId, properties.id))
    .where(and(
      eq(ownerActivity.organizationId, organizationId),
      eq(ownerActivity.ownerId, ownerId)
    ))
    .orderBy(desc(ownerActivity.createdAt))
    .limit(50);

    if (propertyId) {
      query = query.where(eq(ownerActivity.propertyId, propertyId));
    }
    
    if (activityFilter && activityFilter !== 'all') {
      query = query.where(eq(ownerActivity.type, activityFilter));
    }

    return await query;
  }

  // Get AI suggestions for owner
  async getOwnerAISuggestions(organizationId: string, ownerId: string): Promise<any[]> {
    ...
    return await db.select()
      ...
      .where(and(
        ...
        inArray(aiSuggestions.propertyId, propertyIds)
      ))
      .orderBy(desc(aiSuggestions.createdAt));
  }

  // Get financial breakdown for owner
  async getOwnerFinancialBreakdown(organizationId: string, ownerId: string, period: string, customRange?: { start: string; end: string }): Promise<any> {
    ...
    // Get financial records for this owner
    const financialBreakdown = await db.select({
    ...
    .where(and(
      eq(financialRecords.organizationId, organizationId),
      eq(financialRecords.ownerId, ownerId),
      gte(financialRecords.createdAt, startDate.toISOString()),
      lte(financialRecords.createdAt, endDate.toISOString())
    ))
    ...
    const recentTransactions = await db.select({
      ...
      propertyName: properties.name,
      source: financialRecords.source
    })
    .from(financialRecords)
    ...
    financialBreakdown.forEach(record => {
      if (record.type === 'income' && record.category === 'rental') {
        rentalIncome += record.totalAmount;
      } else if (record.type === 'expense' && record.category === 'commission') {
        managementCommission += record.totalAmount;
      } else if (record.type === 'expense') {
        if (record.category === 'maintenance') {
          expenses.maintenance += record.totalAmount;
        } else if (record.category === 'addon') {
          expenses.addons += record.totalAmount;
        } else if (record.category === 'utility') {
          expenses.utilities += record.totalAmount;
        } else if (record.category === 'welcome_pack') {
          expenses.welcomePacks += record.totalAmount;
        } else {
          expenses.other += record.totalAmount;
        }
      }
    });

    const netBalance = rentalIncome - managementCommission - Object.values(expenses).reduce((sum, val) => sum + val, 0);
  ...
  async updateAISuggestionStatus(organizationId: string, suggestionId: number, status: string, approvedBy?: string): Promise<any> {
    ...
    return await db.update(aiSuggestions)
      .set(updateData)
      .where(and(
  ...
  // Log owner activity
  async logOwnerActivity(organizationId: string, ownerId: string, activity: {
    propertyId?: number;
    type: string;
    title: string;
    description: string;
    ...
    return await db.insert(ownerActivity)
      .values({
        organizationId,
        ownerId,
        ...activity,
