...
async function getLodgifyApiKey(organizationId: string): Promise<string> {
  ...
  if (apiKeyRecord.length > 0 && apiKeyRecord[0].encryptedValue) {
    ...
    console.log(`[Lodgify] Using database API key for organization: ${organizationId}`);
  ...
  if (envKey) {
    console.log(`[Lodgify] WARNING: Using fallback environment variable for org ${organizationId}. Configure org-specific key in production.`);
    return envKey;
  }
